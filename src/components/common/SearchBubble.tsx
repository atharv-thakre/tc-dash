  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-indigo-50 dark:bg-indigo-950/70 text-indigo-600 dark:text-indigo-400 border border-indigo-200/60 dark:border-indigo-800/60 hover:bg-indigo-100 dark:hover:bg-indigo-900 text-xs font-semibold select-none cursor-pointer active:scale-95 transition-all shadow-2xs whitespace-nowrap group shrink-0"
      >
        <RotateCw className="w-3 h-3 text-indigo-500 group-hover:rotate-180 transition-transform duration-300" />
        <span>Field: <strong className="font-bold text-indigo-700 dark:text-indigo-300">{currentField.label}</strong></span>
        <span className="text-[10px] text-indigo-400 dark:text-indigo-500 font-mono bg-indigo-100 dark:bg-indigo-900/80 px-1.5 py-0.5 rounded-full ml-0.5">
      ...
      <div className="relative flex-1 flex items-center min-w-0">
        <input
          ...
          className="w-full bg-transparent border-0 text-xs text-gray-900 dark:text-white placeholder:text-gray-400 focus:outline-none focus:ring-0 px-2 py-1 min-w-0"
        />
        ...
        {searchValue && (
          <button
            ...
            className="p-1 rounded-full text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors mr-1"
            title="Clear search"
          >
      ...
      <button
        ...
        disabled={isLoading}
        className="flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold shadow-xs hover:shadow transition-all active:scale-95 cursor-pointer disabled:opacity-60 shrink-0"
      >
        <Search className={`w-3.5 h-3.5 ${isLoading ? 'animate-spin' : ''}`} />
        <span>{isLoading ? 'Querying...' : 'Search'}</span>
      </button>
    </div>
  );
};
